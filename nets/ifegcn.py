...
from torch_geometric.utils import add_remaining_self_loops, to_dense_batch, subgraph
...
# WithRelu and Without dropout
class GCNConvWithReLU(MessagePassing):
    r"""The graph convolutional operator from the `"Semi-supervised
    Classification with Graph Convolutional Networks"
    <https://arxiv.org/abs/1609.02907>`_ paper
    .. math::
    ...
    def __init__(self, in_channels: int, out_channels: int,
        ...
        kwargs.setdefault('aggr', 'add')
        super(GCNConvWithReLU, self).__init__(**kwargs)
...
class GCNExtractor(torch.nn.Module):
    def __init__(self, input_dim, feat_dim, normalize=True, is_add_self_loops=True, dropout_rate=0.5):
        ...
        # self.conv1 = [GCNConvWithReLU(input_dim, feat_dim, cached=False, normalize=normalize)]
