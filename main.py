...
from models.Trainer import NodeClassificationTrainer
...
def run(args):
    ...
    for _ in range(args.repetitions):
        ...
        if args.loss_type == 'ce':
            criterion = CrossEntropy().to(args.device)
        else:
            criterion = None
        ...
        trainer = NodeClassificationTrainer(
            ...
            multi_objective_optimizer=DWA(T=args.DMA_T, device=args.device),
            writer=writer,
            n_cls=n_cls,
            training_args=args)
